...
export default function EmergencyPage() {
  ...
  return (
    <div className="max-w-md mx-auto mt-6 bg-white p-6 rounded shadow">
      <h2 className="text-2xl mb-4">Emergency Request</h2>
      <select value={bloodGroup} onChange={(e)=>setBloodGroup(e.target.value)} className="w-full p-2 border rounded mb-4">
        {["A+","A-","B+","B-","O+","O-","AB+","AB-"].map(b => <option key={b}>{b}</option>)}
      </select>
      <button disabled={submitting} onClick={handleSOS} className="w-full py-2 bg-red-600 text-white rounded">
